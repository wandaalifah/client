<template>
  <DashboardLayout>
    <div id="home" class="relative mx-36 mt-32 pt-10">
      <h1 class="font-semibold text-6xl text-blue-900 w-2/3">SISTEM REKOMENDASI <br> PEMILIHAN MITRA <br> MAGANG</h1>
      ...
      <div class="absolute top-0 right-0">
        <img src="/src/assets/amico.svg" alt="icon" class="flex justify-end">
      </div>
    </div>
    <!-- Statistik -->
    ...
    <div id="rekomendasi" class="mx-36 mb-24">
      <h2 class="font-semibold text-blue-900 text-3xl flex justify-center">Rekomendasi Mitra Magang</h2>
      <div class="mt-20">
        <div class="flex justify-center ">
          <p>Tuliskan NIM anda</p>
        </div>
        <div class="flex justify-center">
          <input v-model="nim" class="border-2 border-purple-400 rounded h-10 w-60 p-4" placeholder="4611419035">
        </div>
      </div>
      <div class="flex justify-center mt-4">
        <p>Tuliskan Minat Anda</p>
      </div>
      <div class="flex justify-center ">
        <input v-model="minat" class="border-2 border-purple-400 rounded h-10 w-60 p-4" placeholder="Frontend Web Developer, UI UX Designer">
      </div>
      <div class="flex justify-center mt-4">
...
export default {
  ...
  setup() {
    ...
    const processRec = () => {
      mitraStore.getMagangRecommendation(nim.value, minat.value).then(result => {
        recs.value = result
    ...
    return {nim, minat, recs, successMessage, hasilMessage, processRec, chartData}
  }
}
</script>
